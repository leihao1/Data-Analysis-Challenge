...
"""
def evaluate_regressors(X_train, y_train, X_test, y_test, all_regrs, regr_names, file_name, output_path=None):
...
def evaluate_classifiers(X_test, y_test, all_clfs, clf_names, file_name, output_path=None):
...
#     plt.figure(figsize=(10,8))
    ...
    for clf, clf_name in zip(all_clfs, clf_names):
        ...
        #multi classes
            ...
            plt.plot(fpr["micro"], tpr["micro"], 'b', label = '%s (Micro AUC = %0.3f)' % (clf_name,roc_auc["micro"]), c=np.random.rand(3,))
            plt.legend(loc = 'lower right')        
    return roc_scores
...
#     plt.scatter(comp1,comp2,c=y,cmap=color_map,alpha=1)

    #plot labels
    labels = np.array(classes)[y]
    class_num = set()
    for x1,x2,c,l in zip(comp1,comp2,color_map(y),labels):
#         if len(class_num)==len(classes):
#             break
        plt.scatter(x1,x2,c=[c],label=l,alpha=0.7)
        class_num.add(l)
